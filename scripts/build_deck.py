...
def gather_facts(context) -> Facts:
    ...
    company_totals = segment_rows.groupby("company_clean")["value_t2"].sum().sort_values(
        ascending=False
    )
    ...
    stability = run_sensitivity(
        context.enriched, level="sub_segment", framework=context.framework, top_k=3
    ).stability.head(5).to_dict("records")
    ...
    grouped = (
        focal.groupby("brand_root")[["MAT FEB'25", "MAT FEB'26"]]
        .sum()
        .nlargest(5, "MAT FEB'26")
    )
    franchises = [
...
def _content_slide(presentation, eyebrow: str, title: str, *, subtitle: str = ""):
    ...
    _text(slide, MARGIN, Inches(0.44), Inches(11.0), Inches(0.3), eyebrow.upper(),
          size=11, bold=True, colour=TEAL, font=BODY_FONT)

    lines = sum(
        max(1, -(-len(line) // TITLE_CHARS_PER_LINE)) for line in title.split("\n")
    )
    title_height = Inches(lines * TITLE_SIZE * 1.1 / 72.0 + 0.06)
    title_top = Inches(0.78)
    _text(slide, MARGIN, title_top, Inches(12.1), title_height, title,
          size=TITLE_SIZE, bold=True, colour=INK, font=HEAD_FONT, line_spacing=1.1)

    if subtitle:
        _text(slide, MARGIN, title_top + title_height + Inches(0.14),
              Inches(11.8), Inches(0.44), subtitle,
              size=13, colour=MUTED, font=BODY_FONT)
    return slide


def _stat(slide, left, top, width, value: str, label: str, *, colour=TEAL, value_size=30):
    _text(slide, left, top, width, Inches(0.52), value,
          size=value_size, bold=True, colour=colour, font=HEAD_FONT)
    _text(slide, left, top + Inches(0.52), width, Inches(0.6), label,
          size=11, colour=MUTED, font=BODY_FONT, line_spacing=1.16)
...
def _footnote(slide, text: str):
    _text(slide, MARGIN, Inches(6.94), Inches(12.1), Inches(0.32), text,
          size=9.5, colour=MUTED, font=BODY_FONT)


def _table(slide, left, top, width, headers, rows, *, col_ratios, row_h=Inches(0.42),
           head_h=Inches(0.36), size=11.5, accent_col=None):
    ...
    for index, header in enumerate(headers):
        _text(slide, x, top, widths[index], head_h, header, size=10, bold=True,
              colour=MUTED, font=BODY_FONT,
    ...
    for row in rows:
        ...
        for index, cell in enumerate(row):
            ...
            _text(slide, x, y, widths[index] - Inches(0.12), row_h, str(cell),
                  size=size,
...
def slide_cover(presentation, facts: Facts, subtitle: str) -> None:
    ...
    _text(slide, Inches(1.05), Inches(1.68), Inches(7.0), Inches(0.34),
          "ASCEND SEASON 4  ??  CASE STUDY RESPONSE", size=11.5, bold=True,
          colour=TEAL_SOFT, font=BODY_FONT)
    _text(slide, Inches(1.05), Inches(2.16), Inches(7.1), Inches(1.9),
          "Where Cipla can win\nin the India Cardiac market",
          size=40, bold=True, colour=WHITE, font=HEAD_FONT, line_spacing=1.08)
    _text(slide, Inches(1.05), Inches(4.12), Inches(6.9), Inches(0.9), subtitle,
          size=15, colour=RGBColor(0xB9, 0xC6, 0xD1), font=BODY_FONT, line_spacing=1.3)
    _text(slide, Inches(1.05), Inches(6.42), Inches(7.0), Inches(0.32),
          "AI-enabled prioritisation with integrated trend analytics  ??  MAT February 2026",
          size=11, colour=MUTED, font=BODY_FONT)
    ...
    for value, label in metrics:
        _text(slide, Inches(9.28), top, Inches(3.6), Inches(0.55), value,
              size=33, bold=True, colour=TEAL_SOFT, font=HEAD_FONT)
        _text(slide, Inches(9.28), top + Inches(0.63), Inches(3.6), Inches(0.3), label,
              size=11.5, colour=MUTED, font=BODY_FONT)
        top += Inches(1.17)
...
def slide_finding(presentation, facts: Facts) -> None:
    ...
    stats = [
        ...
        (f"{facts.market_real:.1f}%", f"real demand growth\nof {facts.market_growth:.1f}% reported", SLATE),
        (f"{facts.cipla_share:.2f}%", f"Cipla share\nrank {facts.cipla_rank} of {facts.company_count}", TERRACOTTA),
    ]
    left = MARGIN
    ...
    _text(slide, MARGIN, Inches(3.95), Inches(5.2), Inches(0.3), "HOW IT WORKS",
          size=10.5, bold=True, colour=TEAL, font=BODY_FONT)
    method = (
    ...
    _text(slide, MARGIN, Inches(4.32), Inches(5.35), Inches(2.3), method,
          size=12, colour=SLATE, font=BODY_FONT, line_spacing=1.22)

    _picture(slide, "priority-matrix-sub-segment", Inches(6.35), Inches(2.06), Inches(6.4))
    _footnote(slide, "Source: Cardiac prescription audit, MAT Feb 2026. Bubble area is market value.")
...
def slide_priorities(presentation, facts: Facts) -> None:
    ...
    cards = [
        (
            MARGIN,
            "PRIORITY 1", TEAL, TEAL_SOFT,
            "Statin combinations,\nvia ezetimibe",
            [
                ("Sub-segment", f"{statins['value_t2']:,.0f} cr, +{statins['value_yoy']*100:.1f}%, real +{statins['real_growth']*100:.1f}%"),
                ("Structure", f"HHI {statins['hhi']:,.0f}, {int(statins['n_players'])} players, leader {statins['leader_share']*100:.0f}%"),
                ("Cipla today", f"{statins['focal_value_t2']:,.1f} cr, {statins['focal_share_t2']*100:.2f}%, +{statins['focal_yoy']*100:.1f}%"),
                ("The target", f"Rosuvastatin + ezetimibe: {ezetimibe['value_t2']:,.0f} cr, +{ezetimibe['value_yoy']*100:.0f}%"),
                ("Right to win", "Rosulip is the fastest-growing Cipla franchise; Rosulip EZ is launched"),
            ],
        ),
        (
            Inches(6.92),
            "PRIORITY 2", TERRACOTTA, TERRA_SOFT,
            "Cilnidipine combinations,\nvia Cresar",
            [
                ("Franchise", f"{cilnidipine['value_t2']:,.0f} cr across plain and combination packs, +{cilnidipine['value_yoy']*100:.1f}%"),
                ("Cipla today", f"{cilnidipine['focal_share_t2']*100:.2f}% share, {cilnidipine['focal_value_t2']:,.1f} cr, growing {cilnidipine['focal_yoy']*100:.0f}%"),
                ("Where to enter", f"Not the plain molecule. The dual sits at HHI {cilni_telmi['hhi']:,.0f}; the triples are more open"),
                ("The target", f"Cilnidipine + telmisartan: {cilni_telmi['value_t2']:,.0f} cr, +{cilni_telmi['value_yoy']*100:.0f}%, real +{cilni_telmi['real_growth']*100:.0f}%"),
                ("Right to win", "Cresar carries telmisartan; Cresar LN already sells in this space"),
            ],
        ),
    ]
    ...
    for left, tag, accent, soft, heading, rows in cards:
        ...
        _text(slide, left + Inches(0.3), Inches(2.58), Inches(5.2), Inches(0.24), tag,
              size=10, bold=True, colour=accent, font=BODY_FONT)
        _text(slide, left + Inches(0.3), Inches(2.84), Inches(5.2), Inches(0.62), heading,
              size=17, bold=True, colour=INK, font=HEAD_FONT, line_spacing=1.06)
        top = Inches(3.52)
        for label, value in rows:
            _text(slide, left + Inches(0.3), top, Inches(1.34), Inches(0.36), label,
                  size=10.5, bold=True, colour=MUTED, font=BODY_FONT)
            _text(slide, left + Inches(1.68), top, Inches(3.82), Inches(0.46), value,
                  size=11.5, colour=INK, font=BODY_FONT, line_spacing=1.14)
            top += Inches(0.49)

    _text(slide, MARGIN, Inches(6.22), Inches(2.0), Inches(0.26), "TRADE-OFFS RESOLVED",
          size=10.5, bold=True, colour=TEAL, font=BODY_FONT)
    tradeoffs = [
    ...
    for line in tradeoffs:
        ...
        _text(slide, MARGIN + Inches(0.18), top, Inches(12.0), Inches(0.28), line,
              size=10.5, colour=SLATE, font=BODY_FONT)
        top += Inches(0.29)
...
def slide_whitespace(presentation, facts: Facts) -> None:
    ...
    _text(slide, MARGIN, Inches(5.6), Inches(6.5), Inches(0.6),
          ...
          size=11.5, colour=SLATE, font=BODY_FONT, line_spacing=1.24)

    left = Inches(7.36)
    _text(slide, left, Inches(2.3), Inches(5.4), Inches(0.28), "STOP FUNDING",
          size=10.5, bold=True, colour=TERRACOTTA, font=BODY_FONT)

    reasons = [
        (facts.stop[0], "Reported growth is positive, real demand and volume are both negative. Cipla is declining inside it."),
        (facts.stop[1], "Real growth is negative. Structural decline, not a cycle."),
        (facts.stop[2], "Fastest growth in the market and the most closed. In-license or leave it."),
    ]
    top = Inches(2.66)
    for row, reason in reasons:
        _card(slide, left, top, Inches(5.4), Inches(1.06), fill=WHITE, accent=TERRACOTTA)
        _text(slide, left + Inches(0.26), top + Inches(0.14), Inches(3.3), Inches(0.28),
              _short(row["space_label"], 34), size=13, bold=True, colour=INK, font=HEAD_FONT)
        _text(slide, left + Inches(0.26), top + Inches(0.44), Inches(4.9), Inches(0.5),
              reason, size=10.5, colour=SLATE, font=BODY_FONT, line_spacing=1.16)
        _text(slide, left + Inches(3.72), top + Inches(0.14), Inches(1.44), Inches(0.28),
              f"real {row['real_growth']*100:+.1f}%", size=11.5, bold=True,
              colour=TERRACOTTA, font=BODY_FONT, align=PP_ALIGN.RIGHT)
        top += Inches(1.18)

    _card(slide, left, Inches(6.22), Inches(5.4), Inches(0.72), fill=TEAL_SOFT, accent=TEAL)
    _text(slide, left + Inches(0.26), Inches(6.36), Inches(4.94), Inches(0.5),
          "The portfolio is optimised for a market that has already moved. "
          "Concentrating on two configurations beats defending nine.",
          size=11.5, bold=True, colour=INK, font=BODY_FONT, line_spacing=1.18)


def slide_framework(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "Appendix A1", "The prioritisation framework",
    ...
    _table(slide, left, Inches(2.34), Inches(5.5), ["Pillar", "Weight", ""], rows,
           col_ratios=[2.0, 0.8, 3.2], row_h=Inches(0.66), size=11)

    _text(slide, left, Inches(5.5), Inches(5.5), Inches(0.28), "THE GATE",
          size=10.5, bold=True, colour=TEAL, font=BODY_FONT)
    _text(slide, left, Inches(5.84), Inches(5.5), Inches(1.0),
...
def slide_robustness(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "Appendix A2", "How much survives a different framework",
    ...
    _table(slide, left, Inches(2.4), Inches(5.24), ["Sub-segment", "In top 3", "Mean rank"],
           rows, col_ratios=[2.6, 1.0, 1.0], accent_col=1)

    _text(slide, left, Inches(5.2), Inches(5.24), Inches(1.5),
          ...
          "than conclusions.",
          size=11.5, colour=SLATE, font=BODY_FONT, line_spacing=1.26)


def slide_sources(presentation, facts: Facts, context) -> None:
    slide = _content_slide(
        presentation, "Appendix A3", "Sources for external data",
    ...
    rows = [
        [c["id"], c["title"][:54], c["publisher"][:38]]
        for c in citations
    ]
    _table(slide, MARGIN, Inches(2.32), Inches(12.1), ["", "Signal", "Publisher"], rows,
           col_ratios=[0.5, 5.6, 3.4], row_h=Inches(0.33), size=10.5)

    _text(slide, MARGIN, Inches(6.42), Inches(12.1), Inches(0.5),
          ...
          size=11, colour=MUTED, font=BODY_FONT, line_spacing=1.22)


def slide_limits(presentation, facts: Facts) -> None:
    slide = _blank(presentation, INK)
    _rect(slide, 0, 0, Inches(0.16), SLIDE_H, TERRACOTTA)
    _text(slide, MARGIN + Inches(0.3), Inches(0.72), Inches(11.0), Inches(0.3),
          "APPENDIX A4", size=11, bold=True, colour=TERRACOTTA, font=BODY_FONT)
    _text(slide, MARGIN + Inches(0.3), Inches(1.08), Inches(11.4), Inches(0.7),
          "What this analysis cannot tell you", size=30, bold=True, colour=WHITE,
          font=HEAD_FONT)

    limits = [
        ("Two years of history", "Two annual observations support a direction, not a cycle. Five-year projections are structured extrapolation with an explicit mean-reversion assumption."),
        ("No patent or regulatory status", "The framework can see that a space is originator-dominated. It cannot say whether entry is legally possible. Any entry decision needs a freedom-to-operate review."),
        ("No cost or margin data", "Every conclusion is about revenue opportunity. Whether a space is profitable to enter is not answerable here."),
        ("Retail audit only", "Institutional and government-tender channels are absent, which matters given the scale of public screening programmes."),
        ("Curated external signals", "Fourteen documents chosen for relevance, not a systematic review. Their influence is bounded by design for exactly that reason."),
    ]
    top = Inches(2.16)
    for heading, body in limits:
        _rect(slide, MARGIN + Inches(0.3), top + Inches(0.06), Inches(0.05), Inches(0.2), TERRACOTTA)
        _text(slide, MARGIN + Inches(0.52), top, Inches(3.3), Inches(0.3), heading,
              size=13.5, bold=True, colour=WHITE, font=HEAD_FONT)
        _text(slide, Inches(4.72), top, Inches(8.1), Inches(0.6), body,
              size=11.5, colour=RGBColor(0xB9, 0xC6, 0xD1), font=BODY_FONT, line_spacing=1.2)
        top += Inches(0.94)

    _text(slide, MARGIN + Inches(0.3), Inches(6.9), Inches(12.0), Inches(0.3),
          "Stated here rather than buried, because the recommendations should be read against it.",
          size=11, italic=True, colour=MUTED, font=BODY_FONT)
...
def slide_architecture(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "How the agent works", "The model never calculates a number",
        subtitle="Reasoning and arithmetic are separated, and a verifier stands between them.",
    )

    stages = [
        ("01", "Ingest", f"{facts.sku_rows:,} packs validated against a fixed schema, normalised into a warehouse with a SHA-256 of the source."),
        ("02", "Construct", f"{facts.spaces_scored} opportunity spaces across six levels, including molecule franchises that span the reporting hierarchy."),
        ("03", "Score", "Four pillars from percentile-ranked metrics, with a right-to-win gate. Deterministic and reproducible."),
        ("04", "Retrieve", f"{facts.signals} cited external signals linked to spaces, bounded and centred so universal evidence moves nothing."),
        ("05", "Narrate", "The model plans, calls eleven tools and writes prose over a finished evidence pack."),
        ("06", "Verify", "Every number in the draft is matched against the evidence. Unmatched figures are rejected and rewritten."),
    ]
    left = MARGIN
    top = Inches(2.5)
    for index, (number, title, body) in enumerate(stages):
        ...
        _text(slide, x + Inches(0.28), y + Inches(0.18), Inches(0.6), Inches(0.3), number,
              size=15, bold=True, colour=TEAL, font=HEAD_FONT)
        _text(slide, x + Inches(0.92), y + Inches(0.18), Inches(2.7), Inches(0.3), title,
              size=15, bold=True, colour=INK, font=HEAD_FONT)
        _text(slide, x + Inches(0.28), y + Inches(0.6), Inches(3.34), Inches(1.0), body,
              size=11, colour=SLATE, font=BODY_FONT, line_spacing=1.2)

    _footnote(slide, "Guardrails: scope control before any tool runs, numeric grounding and "
...
def slide_demand_vs_price(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "Reading the data", "Seven of thirteen points of growth are price",
    ...
    left = Inches(7.7)
    _text(slide, left, Inches(2.4), Inches(5.05), Inches(2.2),
          ...
          size=12.5, colour=SLATE, font=BODY_FONT, line_spacing=1.3)

    _card(slide, left, Inches(4.86), Inches(5.05), Inches(1.62), fill=TERRA_SOFT, accent=TERRACOTTA)
    _text(slide, left + Inches(0.28), Inches(5.04), Inches(4.5), Inches(0.28),
          "WHAT THIS CHANGES", size=10, bold=True, colour=TERRACOTTA, font=BODY_FONT)
    _text(slide, left + Inches(0.28), Inches(5.34), Inches(4.5), Inches(1.0),
          ...
          size=11.5, colour=INK, font=BODY_FONT, line_spacing=1.2)


def slide_franchise_view(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "Beyond the reporting hierarchy",
    ...
    left = Inches(7.6)
    _text(slide, left, Inches(2.36), Inches(5.15), Inches(0.3),
          "THE CILNIDIPINE CASE", size=10.5, bold=True, colour=TEAL, font=BODY_FONT)
    _text(slide, left, Inches(2.7), Inches(5.15), Inches(1.4),
          ...
          size=12, colour=SLATE, font=BODY_FONT, line_spacing=1.26)

    cilnidipine = facts.priorities[2]
    cilni_telmi = facts.priorities[3]
    rows = [
        ["Whole franchise", f"{cilnidipine['value_t2']:,.0f}", f"{cilnidipine['value_yoy']*100:+.0f}%", f"{cilnidipine['hhi']:,.0f}"],
        ["Dual with telmisartan", f"{cilni_telmi['value_t2']:,.0f}", f"{cilni_telmi['value_yoy']*100:+.0f}%", f"{cilni_telmi['hhi']:,.0f}"],
    ]
    _table(slide, left, Inches(4.3), Inches(5.15), ["Layer", "Cr", "Growth", "HHI"], rows,
           col_ratios=[2.3, 0.9, 0.9, 0.9], row_h=Inches(0.44), size=11.5)

    _card(slide, left, Inches(5.66), Inches(5.15), Inches(1.06), fill=TEAL_SOFT, accent=TEAL)
    _text(slide, left + Inches(0.26), Inches(5.84), Inches(4.66), Inches(0.76),
          ...
          size=11.5, bold=True, colour=INK, font=BODY_FONT, line_spacing=1.2)


def slide_cipla_today(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "The starting position",
    ...
    left = Inches(9.28)
    _text(slide, left, Inches(2.4), Inches(3.4), Inches(0.28), "FRANCHISES",
          size=10.5, bold=True, colour=TEAL, font=BODY_FONT)
    rows = [
        [name.title(), f"{value:,.0f}", f"{growth:+.1f}%"]
        for name, value, growth in facts.franchises
    ]
    _table(slide, left, Inches(2.76), Inches(3.5), ["Brand", "Cr", "Growth"], rows,
           col_ratios=[1.7, 0.8, 0.9], row_h=Inches(0.42), size=11.5)

    _text(slide, left, Inches(5.5), Inches(3.5), Inches(1.2),
...
def slide_forecast(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "The three-to-five year view",
    ...
    left = Inches(8.3)
    _text(slide, left, Inches(2.4), Inches(4.4), Inches(0.28), "METHOD",
          size=10.5, bold=True, colour=TEAL, font=BODY_FONT)
    _text(slide, left, Inches(2.74), Inches(4.4), Inches(2.6),
    ...
    _text(slide, left + Inches(0.26), Inches(5.84), Inches(3.9), Inches(0.76),
...
def slide_execution(presentation, facts: Facts) -> None:
    slide = _content_slide(
        presentation, "Execution", "What happens, and in what order",
        subtitle="Sequenced so the shorter-cycle move funds attention for the longer one.",
    )

    phases = [
        ("MONTHS 0-12", TEAL, "Scale the ezetimibe extension", [
        ...
        ("MONTHS 12-24", SLATE, "Build the cilnidipine combination line", [
        ...
        ("CONTINUOUS", TERRACOTTA, "Release the funding", [
    ...
    for tag, accent, heading, points in phases:
        ...
        _text(slide, left + Inches(0.28), Inches(2.62), Inches(3.3), Inches(0.26), tag,
              size=10, bold=True, colour=accent, font=BODY_FONT)
        _text(slide, left + Inches(0.28), Inches(2.92), Inches(3.34), Inches(0.66), heading,
              size=15.5, bold=True, colour=INK, font=HEAD_FONT, line_spacing=1.08)
        top = Inches(3.78)
        for point in points:
            _rect(slide, left + Inches(0.28), top + Inches(0.06), Inches(0.05), Inches(0.14), accent)
            _text(slide, left + Inches(0.46), top, Inches(3.18), Inches(0.66), point,
                  size=11, colour=SLATE, font=BODY_FONT, line_spacing=1.2)
            top += Inches(0.72)
        left += Inches(4.12)

    _card(slide, MARGIN, Inches(6.18), Inches(12.1), Inches(0.76), fill=TERRA_SOFT, accent=TERRACOTTA)
    _text(slide, MARGIN + Inches(0.28), Inches(6.34), Inches(11.5), Inches(0.5),
...
def slide_close(presentation, facts: Facts) -> None:
    slide = _blank(presentation, INK)
    _rect(slide, 0, 0, Inches(0.16), SLIDE_H, TEAL)
    _text(slide, Inches(1.4), Inches(2.24), Inches(10.4), Inches(0.3),
          "THE IMPLICATION", size=11.5, bold=True, colour=TEAL_SOFT, font=BODY_FONT)
    _text(slide, Inches(1.4), Inches(2.68), Inches(10.6), Inches(1.9),
          "Cipla cannot win this market by covering it.\n"
          "It can win two configurations inside it,\nboth next to brands it already owns.",
          size=30, bold=True, colour=WHITE, font=HEAD_FONT, line_spacing=1.18)
    _text(slide, Inches(1.4), Inches(4.86), Inches(10.4), Inches(0.9),
          "And it should stop funding the legacy positions that make the portfolio look "
          "broader than its right to win actually is.",
          size=15, colour=RGBColor(0xB9, 0xC6, 0xD1), font=BODY_FONT, line_spacing=1.28)
...
def build_round_one(facts: Facts, context) -> Path:
    ...
    slide_cover(presentation, facts,
                "Three slides. Two priorities, the trade-offs behind them, and where to stop "
                "spending.")
...
def build_detailed(facts: Facts, context) -> Path:
    presentation = _new_presentation()
    slide_cover(presentation, facts,
                "The full argument: how the agent works, what it found, and what Cipla should "
                "do about it.")
